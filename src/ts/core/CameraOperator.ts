...
export class CameraOperator implements IInputReceiver, IUpdatable
{
	...
	{
		// this.targetRadius = Math.max(1, value);
	...
	public update(timeScale: number): void
	{
		...
		else 
		{
			...
			// this.camera.position.x = -67.5867871393042;
			// this.camera.position.y = 22.961888031566378;
			// this.camera.position.z = 30.693521780679333;

			this.radius = THREE.MathUtils.lerp(this.radius, this.targetRadius, 0.1);
			
			if(this.prevPosition.x === this.camera.position.x && this.prevPosition.y === this.camera.position.y && this.prevPosition.z === this.camera.position.z){
			}else{
				// console.log(this.camera.position)
				this.prevPosition.x = this.camera.position.x;
				this.prevPosition.y = this.camera.position.y;
				this.prevPosition.z = this.camera.position.z;
				// document.getElementById('position_camera').innerHTML = 'x: ' + this.camera.position.x + ', y: ' + this.camera.position.y + ', z: ' + this.camera.position.z;
				// console.log(`${this.target.x},${this.target.y},${this.target.z},${this.radius},${this.theta},${this.phi}`)
				sessionStorage.setItem('position', `${this.target.x},${this.target.y},${this.target.z},${this.radius},${this.theta},${this.phi}`);
				// console.log(`
			...
			// this.camera.position.y = this.target.y + this.radius * Math.sin(this.phi * Math.PI / 1);
